...
class FileList {
  ...
  _refresh () {
    ...
    const matchFiles = async ({ pattern, type, nocache, isBinary }) => {
      if (helper.isUrlAbsolute(pattern)) {
        return { files: [new Url(pattern, type)], pattern, nocache }
      }

      const mg = await new Promise((resolve, reject) => {
        const g = new Glob(pathLib.normalize(pattern), { cwd: '/', follow: true, nodir: true }, (err) => {
          if (err) {
            reject(err)
          } else {
            resolve(g)
      ...
      if (_.isEmpty(mg.found)) {
        ...
        log.debug(`Pattern "${pattern}" matched ${files.length} files.`)
      }

      return { files, pattern, nocache }
    }

    let lastCompletedRefresh = this._refreshing
    lastCompletedRefresh = Promise.resolve().then(async () => {
      const matchResults = await Promise.all(
        this._patterns.map(async (p) => {
          try {
            return await matchFiles(p)
          } catch (err) {
            log.error(`Failed to match files for pattern ${p.pattern}\n${err.stack}`)
            throw err
          }
        })
      )
      await Promise.all(matchResults.map(async ({ files, pattern, nocache }) => {
        if (!helper.isUrlAbsolute(pattern)) {
          if (nocache) {
      ...
      // When we are here the file processing chain is complete.
      // In the case of two fast refresh() calls, the second call
